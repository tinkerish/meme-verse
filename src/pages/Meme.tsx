...
import useMemeStore, { Meme as MemeType } from "../store/formStore";
...
const MyMemePageComponent = () => {
  ...
  const [processedMemes, setProcessedMemes] = useState<MemeType[]>([]);
  const [displayedMemes, setDisplayedMemes] = useState<MemeType[]>([]);
  ...
  const editHandler = (id: string) => navigate(`/add-meme/${id}`);
  const viewDetailsHandler = (id: string) => navigate(`/meme/${id}`);
